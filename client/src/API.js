...
async function startHike(HikeID,PointID,Timestamp) {
  try {
...
async function reachedPointInfo(PointID, HikeID, HikerID) {
  try {
    const response = await fetch((APIURL + '/api/activePoint/getPointReachedInfo'), {
      method: 'POST',
      credentials: 'include',
      body: JSON.stringify({
        "PointID": PointID,
