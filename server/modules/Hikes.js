...
function setHikePicture  (hikeId,path)  {
 ...
 function addHike (Hike)  {
  ...
  return new Promise(async (resolve, reject) => {
    db.run("INSERT INTO Hikes (HikeID, Title, Length, ExpectedTime, Ascent, Difficulty, Start, End, Description, Country, Region, Province, City, GpxFile, AssociatedGuide,Picture ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)",
      [Hike.HikeID, Hike.Title, Hike.Length, Hike.ExpectedTime, Hike.Ascent, Hike.Difficulty, Hike.Start, Hike.End, Hike.Description, Hike.Country, Hike.Region, Hike.Province, Hike.City, Hike.GpxFile, Hike.AssociatedGuide,Hike.Picture ], function (err) {
        if (err)
          reject(err);
        else
